...
\Def{}{Soit $u\in\mathscr{C}^0(\Omega)$. On dit que $u$ est sous-solution de (\ref{EDP}) si $\forall \phi\in\mathscr{C}^2(\Omega)$ tel que $u-\phi$ atteint un maximum en $\bar{x}$, on a 
...
\Def{Solution de viscosité}{Soit $u\in\mathscr{C}^0(\Omega)$. On appelle sur-différentiel d'ordre 2 de $u$ en $\bar{x}$, noté $D^{2+}(\bar{x})$ l'ensemble convexe constitué des couples $(p,M)\in\mathbb{R}^N\times S^N$ tel que :
...
Si $u_\varepsilon\xrightarrow[\varepsilon\to 0]{} u$ dans $\mathscr(\Omega)$, dans le sens où pour tout compact $K\subset\Omega$, $\|u_\varepsilon-u\|_{L^\infty(K)}\xrightarrow[\varepsilon\to 0]{} 0$\\
...
\paragraph{Utilisation du théorème}\begin{enumerate}
	...
	\item On montre que $u_\varepsilon$ est loalement uniformément holderienne ou lipschitzienne
...
Alors $\underline{f}=\bar{f}$ dans $\mathbb{R}$ si et seulement si $f_i$ converge uniformément sur $X$ quand $i\to+\infty$ vers une fonctions continue $f$.\\
...
\label{PC}(PC) Si $u$ scs est sous-solution de (\ref{EDP}), si c sci est sur-solution, et si $u\leq g\leq v$ sur $\partial\Omega$ ($g\in\mathscr{C}^0(\Omega)$), alors $u\leq v$ dans $\Omega$\\
...
\Theo{}{On suppose qu'il existe une sous-solution bérrière $\utilde{u}$ et une sur-solution $\tilde{u}$ de (\ref{EDP-CB}).\\
...
\Coro{Convergence a priori}{On suppos que $u_\varepsilon\to u$ et $F_\varepsilon\to F$ sur les compacts. Alors $u$ solution de (\ref{EDP}).}
