...
On suppose $f$ lipschtizienne par rapport à sa première variable et bornée par rapport à la deuxième : $\exists C>0$
...
$\overline{F}$ est appelé l'hamiltonien effectif.\\
On s'intéresse au problème :
\begin{equation}\label{EC} \tag{EC}
\left\{ \begin{array}{c}
	F\left(y,x, u, p+D_y v\right)=0 y\in\mathbb{R}^N\\
	v\ Y\text{-périodique}
\end{array}\right.
\end{equation}

\Propo{}{Soit $(x,u,p)\in\Omega\times\mathbb{R}\times\mathbb{R}^N$.\\
$\exists !\lambda=\overline{F}(x,u,p)$ tel que $\exists v$ solution de (\ref{EC}).}

\textbf{Remaruque :} $v$ n'est pas unique. On l'appelle le correcteur.

\Lem{}{\begin{enumerate}
	\item $\lim_{|p|\to+\infty}\overline{F}(x,u,p)=+\infty$ uniformément dans $\Omega\times\overline{B(0,L)}$
	\item $r\mapsto F(x,r,p)-\mu r$ est croissante $\forall x,p$
	\item $\overline{F}$ est lipschitzienne sur $\Omega\times B(0,L)\times B(0,L)$ $\forall L>0$
\end{enumerate}
On a ainsi un principe de comparaison à la limite.}
